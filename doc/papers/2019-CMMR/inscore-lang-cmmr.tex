...
\newcommand{\toforest}	{\ensuremath{\mathcal{F}}}
...
\]
The \emph{slash} special value of a tree is used to discriminate the OSC address and the parameters of the messages. We need to define two tree sub-types: 
\begin{description}
\item an $address$ type: noted $t^a$, that denotes a tree part of the OSC address
\item a $data$ type: noted $t^d$, that denotes a tree part of the OSC data
\end{description}
Note that in INScore, an OSC message can be part of the OSC data. Thus a tree can carry both the address and data sub-types.\\
Then we define an operation \toType that transforms a tree in to a \emph{sub-typed} tree:
\[
\left\{
\begin{array}{l}
