...
During the evaluation process, the evaluator goes through every nodes of the expression tree using a depth first post-order traversal, reducing all of them into GMN code. Figure~\ref{fig:classicEval} illustrate this process.
...
Because \sExpr\ are designed for users not used to functional programming, the language should be conceptually safe meaning that it shouldn't be possible to write conceptually wrong expression (infinite recursion definition for example). For that reason, all arguments are referential transparent by default: each arguments are evaluated once and their values are then considered definitive. 
%In particular if an argument refers to the object \OSC{score}, once it have been evaluated its value won't change, even if \OSC{score} is modified.
