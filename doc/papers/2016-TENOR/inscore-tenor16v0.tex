...
\caption{Propagation of dynamic evaluation. \OSC{\prefix{\&}\param{score}} is updated to the actual value of \OSC{score} when re-evaluating, while \OSC{\param{score}} keeps the value computed on the first evaluation. Thus, on re-evaluation the second \OSC{\oper{seq}} should not be computed again. 
...
Using variable arguments, expression trees with arbitrary variable parts can be described: that may be viewed as building a symbolic score with arbitrary aggregation of static and variable parts.
