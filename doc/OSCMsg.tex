...
\title{INScore \\ OSC Messages Reference \\v.1.12}
...
setMsg : 'set' (
	...
	('line' ('xy' x y | 'wa' width angle) (left right ( sizeL sizeR |) |)) |
)
\end{rail}
\index{Set type!svg}
\index{Set type!svgf}
...
\item \OSC{line}: a simple line specified by a point (x,y) expressed in \OSC{scene} coordinate space or by a width and angle. The point form is used to compute a line from (0,0) to (x,y), which is next drawn centered on the scene. An arrow head can be added to the line at the beginning and at the end of the line. This optionals parameters \OSC{left} and \OSC{right} can have the value "none", "triangle", "diamond" or "disk" (default value to "none"). The size of arrow heads can be setted by float numbers \OSC{sizeL} and \OSC{sizeR} for each head (default to 1.0).
\end{itemize}
...
\end{rail}
\index{Set file!filePath}
\index{Set file!urlPath}

Only the \OSC{http:}, \OSC{https:} and \OSC{file:} protocols are currently supported for URLs.
...
\sublevel{The websocket server}
\label{websocketserver}

\label{setwebsocket}

The websocket server object is used to have communication between INScore and distant clients. The object push notification to client (a text message "Screen updated") when his scene is updated. Clients can ask the server for an image by sending "getImage" text message to the server. The server respond by a png image of the scene in a Blob type javascript object.
...
\item \OSC{frequency}: a minimum time in millisecond between two notifications "Screen updated".
\end{itemize}

\seealso the http web server plugin (section \fullref{Httpd}).

\note{} \\
If the http port is already used, the server can't start. The server status can be verified with the \OSC{get status} message.

\example \\
Creating an websocket server using the port \OSC{1234} and have not more one notification per 500 milliseconds. 
