...
\OSC{regexp} represents regular expressions as defined by the Open Sound Control.
...
\begin{figure}[H]
\centering
\includegraphics[width=15cm]{imgs/brushStyle.png}
\caption{Qt brush styles}
...
\begin{itemize}
\item \OSC{brushStyle} controls the brush style (see Figure \ref{fig:brushStyles}).
 \end{itemize}
...
\note{} \\
A master can naturally have several slaves, but we should mention that a slave can as well have several masters. In this case, it would be drawn several times, corresponding to each master's space.
...
\note{} \\
The \values{y} position of a synchronized object remains a free attribute. To control it, you should send \OSC{dy} messages.  
...
Every object includes a static signal node, which OSC address is \OSC{/ITL/scene/.../\textit{object}/signal} which may be viewed as a container for signals. It is also used for \textit{composing signals in parallel} and for \textit{connecting signals to objects' attributes}.
...
\label{ssignal}
Signal messages can be sent to any address with the form \OSC{/ITL/scene/.../\textit{object}/signal/\textit{identifier}}, where \OSC{\textit{identifier}} is a unique signal identifier.
...
%---------------------------------
% signal connections
\subsublevel{The signal connections.}
\label{signalnodemsg}

As said before, the \OSC{signal} node is in charge of storing the simple signals and the signals in parallel, but also to receive the connection messages and to register all connections made at its level of hierarchy. This mechanism is done by the \OSC{connect} message that links a signal to one or more attributes of objects on the same level as the node, and the \OSC{disconnect} message that can break those links.

\begin{rail}
signalNodeMsg : [1] 'connect' connection
		| 'disconnect' ( [2] identifier | [3] connection)
\end{rail}


\begin{itemize}
\item \textbf{[1]} the \OSC{connect} message makes connection between a signal and one or several attributes of one or several objects.
\item \textbf{[2]} the \OSC{disconnect} message with only the \OSC{identifier} parameter disconnects all links with this signal
\item \textbf{[3]} the \OSC{disconnect} message will only delete the connection specified.
\end{itemize}

\begin{rail}
connection : identifier ( objectsAttributes + )
\end{rail}

\begin{rail}
objectsAttributes :  object ( ':' attribute ( | range ) + )
\end{rail}
...
\begin{itemize}
\item \OSC{identifier} is a signal name referring to an existing signal in the \OSC{signal} node. 
\item \OSC{object} is the name of the object (on the same level of hierarchy as the \OSC{signal} node) whose attribute we want to connect.
\item \OSC{attribute} is the name of the attribute we want to connect to the signal (same name as the method used to set the attribute).
\item \OSC{r1} and \OSC{r2} are the optional range bounds to define other bounds for the attribute as the default ones. (there is no rule for their range or their order : r1 < r2 or r2 < r1) They can only be used when setting the connections, but not for disconnection (useless).
\end{itemize}
...
\note{} \\
For now, the connections can only be made to one-dimensional signals
...
\sample{/ITL/scene/signal connect sig1 "rect:red" \\
