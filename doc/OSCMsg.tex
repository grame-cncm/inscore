...
\title{INScore \\ OSC Messages Reference \\v.1.09}
...
\note{} The files accept a local file path, but also a web address (urlPath). In this case, the object is a 1x1 black square, till the data is ready to be drawn.
...
%===============================
%:       URL Files events
\subsublevel{URL files related events}
\label{urlevents}
...
what : 	'success'  
		...
		| 'canceled'
\end{rail}
\index{Interaction!Events!urlSuccess}
\index{Interaction!Events!urlError}
\index{Interaction!Events!urlCanceled}
...
\item \OSC{canceled} is triggered when the URL has been changed during the download
\end{itemize}
