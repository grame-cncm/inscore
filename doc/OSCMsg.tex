...
\newcommand{\note}			{\vspace{2mm}\textbf{\hspace{-1.03cm}\textbf{\textsc{Note }}}}
...
\newcommand{\sample}	[1]			{\vspace{-1mm}\begin{center}\colorbox{mygrey}{
...
\item the \emph{signals} level contains signals i.e. objects that accept data streams and that may be graphically rendered as a scene component (see Signals and Graphic signals section \fullref{graphsig}).
...
\sample{/ITL/scene/myObject set file "myImage.png" // will be translated into \\
...
will give the corresponding \OSC{set} message:
\sample{/ITL/scene/myobject set txt "Hello world!"}

\note The \OSC{get width} and \OSC{get height} messages addressed to components that have no explicit width and height (text, images, etc.) returns 0 as long as the target component has not been drew.
...
\note Error messages are sent as a single string.
...
will load \OSC{/path/to/my/folder/myscript.inscore} into the scene. 
...
The \emph{scene} level provides a static nodes - \OSC{debug}, available at \OSC{/ITL/scene/debug} to help debugging communication and INScore scripts design.
...
\note The \OSC{sync} node doesn't responds to common component messages, but accept the \OSC{get} message. 
...
\note A voice may be distributed on several staves and thus a staff may contain several voices.
...
\note A score object duration is automatically set to the score duration. It can be retrieved using a \OSC{get duration} message.
