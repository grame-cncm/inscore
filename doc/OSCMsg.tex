...
\item \OSC{\$date}: denotes the object date corresponding to mouse pointer position at the time of the event. The detailed date variable is given below: it may be followed by a colon and the name of the mapping to used compute the date . The \OSC{\$date} variable is replaced by its rational value (i.e. two integers values). 
\end{itemize}
