...
\title{INScore \\ OSC Messages Reference \\v.1.14}
...
\item \OSC{gmnstream}: a piano roll defined by a GMN stream (a GMN string that can be written in several times).
\end{itemize}
...
Messages accepted by the components types \OSC{pianoroll | pianorollstream}.

\begin{rail}
pianorollMsg :      'type' ('simple' | 'trajectory')
			| 'keyboard' int32
			| 'autoVoicesColoration' int32
			| 'measureBars' int32
			| 'voiceColor' int32 int32 int32 int32 int32
			| 'pitchLines' ( 'Notes' |)
\end{rail}
\index{Specific messages! pianoroll! type}
...
\begin{itemize}
\item \OSC{type}: set the piano roll display type as 'simple' or 'trajectory'.
\item \OSC{keyboard}: display the keyboard on left of piano roll.
\item \OSC{autoVoicesColoration}: enable voices automatic coloration. If voiceColor is used for a voice, automatic voices coloration do nothing for it.
\item \OSC{measureBars}: Display measure bars on piano roll.
\item \OSC{voiceColor}: set a color for a voice. The parameters are voice number (start to 1), and RGBA color.
\item \OSC{pitchLines}:  Display pitch lines on pianoroll. Parameters are a note list in english notation (A A\# B ...).
\end{itemize}
