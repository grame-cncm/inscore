...
\item \OSC{vdate} sets the current frame. Default value is 0. This value is updated while the video is playing.
\end{itemize}
...
Attribute based events includes the whole set of messages that are supported by an object: \OSC{x}, \OSC{y}, \OSC{color}, etc. but also type specific messages. These events are triggered when a message has been succesfully processed. However, you shoudln't assume that the attribute value has been changed: when a message sets an attribute to it's current value, it is succesully processed and the corresponding event - if any - is triggered. 
...
INScore events supports user defined events. The name of user defined event must be cmposed of capital letters and of numbers only. Other characters are reserved for INScore use.
...
\begin{rail}
eventMsg :  'event' ([1] ( (mouseEvent | touchEvent) float32 float32) | [2] eventName)
\end{rail}
...
\item \textbf{2}: trigger any other event 
\end{itemize}
