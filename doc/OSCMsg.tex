...
The \OSC{signal} node supports only the \OSC{get} message that gives the list of the defined signals, and the \OSC{connect} message that links a signal to one or more attributes of objects on the same level as the node.
...
\begin{rail}
connection : identifier ( objectsAttribute + )
\end{rail}

\begin{rail}
objectsAttribute :  object ':' attribute ( | range )
\end{rail}
...
\item \OSC{r1} and \OSC{r2} are the optional range bounds to define other bounds for the attribute as the default ones.
\end{itemize}
...
/ITL/scene/signal connect sig2 rect:blue image:rotatey[0,360] cursor:date[0,15] }
