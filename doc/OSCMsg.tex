...
\title{Augmented Score Library \\ OSC Messages Reference \\v.0.60}
...
\item \OSC{line}: a simple line specified by its dimension (x,y) expressed in \OSC{scene} coordinate space.
...
A \emph{graphic signal} is a composite signal including a set of 3 parallel signals that control these parameters. Thus the \ASL\ provides messages to create signals and to combine them into \emph{graphic signals}. 
...
\toplevel{ Interaction messages}
\label{interaction}

Interaction messages are user defined messages associated to events and triggered when these events occur. The general form of the message is:
...
\begin{rail}
what : 'mouseDown' | 'mouseUp' | 'mouseEnter' | 'mouseLeave' | 'doubleClick' | 'timeEnter' timezone | 'timeLeave' timezone
\end{rail}
%what : 'mouseDown' | 'mouseUp' | 'mouseEnter' | 'mouseLeave' | 'doubleClick' | 'file' | 'timeEnter' timezone | 'timeLeave' timezone
...
The associated OSC message is any valid OSC message (not restricted to the Interlude message set), with an extended address scheme, supporting IP addresses or host names and udp port number to be specified as OSC addresses prefix. 
\begin{rail} 
oscMsg : (| fullAddress)  OSCAddress ( parameters + )
\end{rail}
