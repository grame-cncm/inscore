...
Synchronization between components is in charge of the static \OSC{sync} node, automatically embedded in each object. Its address is \OSC{/ITL/.../\textit{object}/sync} and it supports messages to add or remove a master / slave relation between components (a master should be allowed to have several slaves, and a slave several masters) or to query the synchronizations state.
\begin{rail}
sync : syncIdentifier 
...
\sample{/ITL/scene/myParent/sync mySlave myMaster}
...
Every object includes a static signal node, which OSC address is \OSC{/ITL/scene/.../\textit{object}/signal} which may be viewed as a container for signals. It is also used for \textit{composing signals in parallel}.

\subsublevel{The signal node messages.}
\label{signalnodemsg}

The \OSC{signal} node supports only the \OSC{get} message that gives the list of the defined signals, the \OSC{connect} message that links a signal to one or more attributes of objects on the same level as the node, and the \OSC{disconnect} message that can break those links.

\begin{rail}
signalNodeMsg : [1] 'get'
		| [2] 'connect' connection
		| 'disconnect' ( [3] identifier | [4] connection)
\end{rail}


\begin{itemize}
\item \textbf{[1]} the \OSC{get} message gives us the list of the signals
\item \textbf{[2]} the \OSC{connect} message makes connection between a signal and one or several attributes of one or several objects.
\item \textbf{[3]} the \OSC{disconnect} message with only the \OSC{identifier} parameter disconnects all links with this signal
\item \textbf{[4]} the \OSC{disconnect} message will only delete the connection specified.
\end{itemize}
