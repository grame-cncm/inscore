...
\tikzstyle{test}=[diamond, aspect=2.5,thick,draw=blue,fill=orange!50,text=blue]
...
\node[test] (testI) at (0,9) {$i = elements.size$ ?};
\node[test] (testJ) at (0,4.5) {$j = elements.size - 1$ ?};
...
\draw[suite] (testIT.north) -- (testJ);
\draw[suite] (testJ.west) -- (-5,4.5) node[midway,fill=white] {no};
