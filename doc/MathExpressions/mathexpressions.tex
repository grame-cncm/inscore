...
%===============================
\subsublevel{Strings}
For \OSC{string} parameters, we introduce a conversion operation \mathstrnum{string} that evaluates to \OSC{int32}, and that is applied depending on the operation type (see Tables \ref{table:mathopstr} and \ref{table:mathopstrnum}). Note that we'll refer to numeric values as \OSC{num} to represent \OSC{int32} or \OSC{float32} that are handled similarly in this context.
...
And finally, a boolean operation on \OSC{string} is evaluates as a boolean operation on \mathstrnum{string}.
