...
\begin{itemize}
\item log window utility provided as a new static node at application level (/ITL/log). (see section \fullref{ITLlog})
\item new \OSC{systemCount} read only attribute for Guido scores (see section \fullref{gmnpage})
\item IMTR gesture follower support (see section \fullref{GF})
