...
\item new brush \OSC{brushStyle} option (see section \fullref{brush})
\item new \OSC{save} method specific to the log window: saves the window content to a file
...
\item log window utility provided as a new static node at application level (/ITL/log). (see section \fullref{ITLlog}). The log window is also available from the viewer 'Tools' menu.
