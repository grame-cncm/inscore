...
\begin{itemize}
\item the signal node is available on any level of hierarchy (as well as the sync node)
\item it is possible to connect a signal to some of the object's attributes
\item new 'connect' and 'disconnect' messages on the signal node
\item new 'write' method supported by the 'log' window
\end{itemize}
...
\item 2 different types of hierarchy : the graphical one with the parent-child relation, 
and the temporal one with the master-slave synchronization that can be done on any level of hierarchy.
\item a slave can have several masters
\item the synchronized objects (slaves) are now automatically exported in an offscreen
to be drawn in the master coordinates. they don't change their proportions, unlike the children.
\item new specification on synchronization
\item some modifications on examples (Turenas and cursors) have been made : some settings of scale were 
...
\item new \OSC{layer} object (see section \fullref{inlineset})
...
\item new \OSC{gmnstream} guido stream object (see section \fullref{inlineset})
\end{itemize}
