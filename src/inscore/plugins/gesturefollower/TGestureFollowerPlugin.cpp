...
bool TGestureFollowerPlugin::load ()
{
	...
	if (QPlugin::load(gflibName)) {
		fNew = TPlugin::resolve<TNewFunction> (gflibNewStr);
		if (fNew == 0) return false;
		fDel = TPlugin::resolve<TVoidVoidFunction> (gflibDelStr);
		if (fDel == 0) return false;

		fStartLearn				= TPlugin::resolve<TVoidIntFunction> (gflibStartLearnStr);
		if (fStartLearn == 0) return false;
		fStopLearn				= TPlugin::resolve<TVoidVoidFunction> (gflibStopLearnStr);
		if (fStopLearn == 0) return false;
		fClear					= TPlugin::resolve<TVoidIntFunction> (gflibClearStr);
		if (fClear == 0) return false;
		fClearAll				= TPlugin::resolve<TVoidVoidFunction> (gflibClearAllStr);
		if (fClearAll == 0) return false;
		fStartFollow			= TPlugin::resolve<TVoidVoidFunction> (gflibStartFollowStr);
		if (fStartFollow == 0) return false;
		fStopFollow				= TPlugin::resolve<TVoidVoidFunction> (gflibStopFollowStr);
		if (fStopFollow == 0) return false;
		fSetLikelihoodWindow	= TPlugin::resolve<TVoidIntFunction> (gflibSetLikelihoodWindowStr);
		if (fSetLikelihoodWindow == 0) return false;
		fGetMaxPhrases			= TPlugin::resolve<TIntVoidFunction> (gflibGetMaxPhrasesStr);
		if (fGetMaxPhrases == 0) return false;
		fGetFrameSize			= TPlugin::resolve<TIntVoidFunction> (gflibGetFrameSizeStr);
		if (fGetFrameSize == 0) return false;
		fGetCapacity			= TPlugin::resolve<TIntVoidFunction> (gflibGetCapacityStr);
		if (fGetCapacity == 0) return false;
		fGetLikelihoodWindow	= TPlugin::resolve<TIntVoidFunction> (gflibGetLikelihoodWindowStr);
		if (fGetLikelihoodWindow == 0) return false;
		fSetTolerance			= TPlugin::resolve<TVoidFloatFunction> (gflibSetToleranceStr);
		if (fSetTolerance == 0) return false;
		fGetTolerance			= TPlugin::resolve<TFloatVoidFunction> (gflibGetToleranceStr);
		if (fGetTolerance == 0) return false;
		fObservation			= TPlugin::resolve<TVoidFloatPIntFunction> (gflibObservationStr);
		if (fObservation == 0) return false;
		fLikeliest				= TPlugin::resolve<TIntVoidFunction> (gflibLikeliestStr);
		if (fLikeliest == 0) return false;
		fWhere					= TPlugin::resolve<TFloatPVoidFunction> (gflibWhereStr);
		if (fWhere == 0) return false;
		fLikelihood				= TPlugin::resolve<TFloatPVoidFunction> (gflibLikelihoodStr);
		if (fLikelihood == 0) return false;
		fSpeed					= TPlugin::resolve<TFloatPVoidFunction> (gflibSpeedStr);
		if (fSpeed == 0) return false;
		fGetPhraseSize				= TPlugin::resolve<TIntIntFunction> (gflibGetPhraseSizeStr);
		if (fGetPhraseSize == 0) return false;
		fGetPhraseData				= TPlugin::resolve<TFloatPIntFunction> (gflibGetPhraseDataStr);
		if (fGetPhraseData == 0) return false;
		fGetState				= TPlugin::resolve<TIntVoidFunction> (gflibGetStateStr);
		if (fGetState == 0) return false;
		fVersionStr				= TPlugin::resolve<TCharPVoidFunction> (gflibVersionStr);
