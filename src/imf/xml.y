...
eltname		: NAME							{ store(eltName, imftext); if (!gReader->newElement(imftext)) ERROR("element error") }
endname		: NAME							{ if (!gReader->endElement(imftext)) ERROR("end element error") }

attribute	: attrname EQ value				{ if (!gReader->newAttribute (attributeName, attributeVal)) ERROR("attribute error") }
attrname	: NAME							{ store(attributeName, imftext); }
value		: QUOTEDSTR						{ store(attributeVal, unquote(imftext)); }
...
cdata		: DATA							{ gReader->setValue (imftext); }

procinstr	: PI							{ gReader->newProcessingInstruction (imftext); }
comment		: COMMENT						{ gReader->newComment (imftext); }
...
versiondec	: SPACE VERSION EQ QUOTEDSTR 		{ store(xmlversion, unquote(imftext)); }
encodingdec	: SPACE ENCODING EQ QUOTEDSTR 		{ store(xmlencoding, unquote(imftext)); }
stdalonedec	: SPACE STANDALONE EQ bool  		{ xmlStandalone = yylval; }
bool		: YES | NO ;

doctype		: DOCTYPE SPACE startname SPACE id GT
startname	: NAME 							{ store(doctypeStart, imftext); }
...
publitteral : QUOTEDSTR 						{ store(doctypePub, unquote(imftext)); }
syslitteral : QUOTEDSTR 						{ store(doctypeSys, unquote(imftext)); }
...
#define yy_delete_buffer	imf_delete_buffer
#define yy_scan_string		imf_scan_string
...
void	yyerror(char *s)	{ gReader->error (s, imflineno); }
