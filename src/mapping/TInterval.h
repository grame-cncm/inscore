...
template<typename T> class TInterval
{
	...
	public :
		typedef std::pair<float,float> Relation;
		...
		inline bool near (const TInterval<float>& s) const	  { return TFloat::eq(fFirst, s.first()) && TFloat::eq(fSecond, s.second()); }
		...
		*/
		inline TInterval<T> unite (const TInterval<T>& s, T epsilon = T(0)) const { 
		...
		inline bool intersect(const TInterval<T>& s) const		{ return !((*this) & s).empty(); }
