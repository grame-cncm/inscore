...
{
	public:
			MouseEventAble(const EventsAble* h) : fEventsHandler(h)	{ T::setAcceptHoverEvents(true); }

	protected:
		const EventsAble * fEventsHandler;
		
		void handleEvent (QPointF pos,  EventsAble::eventype type) const
		{
			const std::vector<SEventMessage>& msgs = fEventsHandler->getMsgs (type);
