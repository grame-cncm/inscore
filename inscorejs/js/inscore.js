...
var IEventAble = (function () {
    ...
    IEventAble.prototype.watch = function (msg) {
        ...
        if (n == 2) {
            var arg = msg.paramArray(2);
            var a = this.event2StringArray(event.value);
...
var IObject = (function () {
    ...
    IObject.prototype.processMsg = function (head, tail, msg) {
        ...
        if (this.match(head)) {
            var osc_1 = OSCAddress.shift(tail);
            if (osc_1.second.length)
                this.fSubNodes.forEach(function (obj) { result |= obj.processMsg(osc_1.first, osc_1.second, msg); });
            else {
                var targets = this.find(osc_1.first);
                ...
                else if (Tools.regexp(osc_1.first)) {
                    result = eMsgStatus.kProcessedNoChange;
                }
                else {
                    result = this.proxy_create(msg, osc_1.first, this).status;
                }
            }
        }
...
var VHtmlSvgView = (function (_super) {
    ...
    VHtmlSvgView.prototype.updateView = function (obj) {
        if (obj.isNewData()) {
            ...
            var target = svg ? svg : this.getHtml();
...
var IGuidoCode = (function (_super) {
    ...
    IGuidoCode.prototype.getSVG = function () { return this.fSVG; };
    ...
    IGuidoCode.prototype.AR2SVG = function (ar) {
        var _this = this;
        var gr = IGuidoCode.fGuidoEngine.ar2gr(ar);
        var svg = IGuidoCode.fGuidoEngine.gr2SVG(gr, 1, false, 0);
        this.getMap(gr);
...
var dropLoader = (function () {
            ...
            for (var i = 0; i < filecount; i++) {
                ...
                */
                ...
                if (ext == kInscoreFile) {
                    ...
                    reader.onloadend = this._processMsg(reader, this.fTargetScene, name_1, this.fExtHandlers[ext]);
                }
            }
        }
    };
    ...
    dropLoader.prototype._processMsg = function (reader, targetScene, name, type) {
        return function () {
            var data = reader.result;
...
var IGuidoPianoRoll = (function (_super) {
    ...
    ;
    IGuidoPianoRoll.prototype.AR2SVG = function (ar) {
        var pr = IGuidoPianoRoll.fEngine.ar2PianoRoll(PianoRollType.kSimplePianoRoll, ar);
        ...
        var svg = IGuidoPianoRoll.fEngine.svgExport(pr, 1, 1);
        this.getPRollMap(pr);
...
var ITextf = (function (_super) {
    ...
    // GETS / SETS VALUES 
    //--------------------------------------------------------------    
    ITextf.prototype.getTextFile = function () { return this.fTextFile; };
    ITextf.prototype.setTextFile = function (file) { this.fTextFile = file; };
    // SET HANDLER
    //--------------------------------------------------------------    
    ...
    ITextf.prototype._setText = function (reader) {
        var _this = this;
        return function () { console.log("_setText: " + reader.result); _this.setText(reader.result); };
    };
    // GETSET METHOD
    //--------------------------------------------------------------    
